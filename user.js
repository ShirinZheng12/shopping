$(function () {
    ...
    $("#xin").each(function (a) {
        $("#xin").eq(a).children("a").click(function () {
            ...
            for (var c = 0; c < 5; c++) {
                if (c <= b) {
                    $("#xin").eq(a).find("a").eq(c).find("img").attr("src", "image/hxin.png")
                } else {
                    $("#xin").eq(a).find("a").eq(c).find("img").attr("src", "image/xin.png")
                }
            }
        })
    });
    ...
    //     var a = "";
    //     a = '<p class="addp"><a href="javascript:updateaddress(${u.getAid()})"  id="readd">修改</a><a href="#" id="deladd">删除</a></p>';
    //     $(this).append(a);
