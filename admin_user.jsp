                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <c:forEach var="u" items="${list}">
	                        <tr>
	                            <td class="tc"><input name="id[]" value="${u.getPhone()}" type="checkbox"></td>
	                            <td> ${u.getPhone() }</td>
                                <td> ${u.getName() }</td>
                                <td> ${u.getPassword()  }</td>
                                <td> <img src="${u.getHeadpoto()}" style="width: 40px;height: 40px;"></td>
                                <td>${u.getStatus() == 1?'否':'是'}</td>
                                <td>
                                    <c:if test="${u.getDate() == null}">
                                      --
                                    </c:if>
                                    <c:if test="${u.getDate() != null}">
                                        ${u.getDate()}
	                            ...
	                            <td>
	                                <a class="link-update" href="admin_touserupdate?id=${u.getPhone()}&cpage=${cpage}">修改</a>
	                              <c:if test="${u.getStatus() == 1 }">
	                                <a class="link-del" href="javascript:Delete('你确定要删除用户【${u.getName()} }】吗？', '/MonkeyShop/manage/admin_douserdel?id=${u.getPhone()}&cpage=${cpage }')">删除</a>
	                              </c:if>
